...
void Ctrl_RUDDER(u8 num, double pwmBuf)
{
        ...
        {
                case 1:
												if(DutyfactorSet_LEFT_UP < (pwmBuf - bb) * kk / aa)
                        {
                                DutyfactorSet_LEFT_UP += dutyfactor_change;
                                TIM_SetCompare1(TIM4, DutyfactorSet_LEFT_UP);
                                if (DutyfactorSet_LEFT_UP >= (pwmBuf - bb) * kk / aa)
                                {
                                        DutyfactorSet_LEFT_UP = (pwmBuf - bb) * kk / aa;
                                        TIM_SetCompare1(TIM4, DutyfactorSet_LEFT_UP);
                                }
                        }
                         else if(DutyfactorSet_LEFT_UP > (pwmBuf - bb) * kk / aa)
                        {
                                DutyfactorSet_LEFT_UP -= dutyfactor_change;
                                TIM_SetCompare1(TIM4, DutyfactorSet_LEFT_UP);
                                 if (DutyfactorSet_LEFT_UP <= (pwmBuf - bb) * kk / aa)
                ...
                case 2:
                        if(DutyfactorSet_RIGHT_UP < (pwmBuf - bb) * kk / aa)
                        {
                                DutyfactorSet_RIGHT_UP += dutyfactor_change;
                                TIM_SetCompare2(TIM4, DutyfactorSet_RIGHT_UP);
                                if (DutyfactorSet_RIGHT_UP >= (pwmBuf - bb) * kk / aa)
                                {
                                        DutyfactorSet_RIGHT_UP = (pwmBuf - bb) * kk / aa;
                                        TIM_SetCompare2(TIM4, DutyfactorSet_RIGHT_UP);
                                }
                        }
                        else if(DutyfactorSet_RIGHT_UP > (pwmBuf - bb) * kk / aa)
                        {
                                DutyfactorSet_RIGHT_UP -= dutyfactor_change;
                                TIM_SetCompare2(TIM4, DutyfactorSet_RIGHT_UP);
                                 if (DutyfactorSet_RIGHT_UP <= (pwmBuf - bb) * kk / aa)
                ...
                case 6:
                        if(DutyfactorSet_HORIZ_RIGHT < (pwmBuf - bb) * kk / aa)
                        {
                                DutyfactorSet_HORIZ_RIGHT += dutyfactor_change;
                                TIM_SetCompare2(TIM3, DutyfactorSet_HORIZ_RIGHT);
                                if (DutyfactorSet_HORIZ_RIGHT >= (pwmBuf - bb) * kk / aa)
                                {
                                        DutyfactorSet_HORIZ_RIGHT = (pwmBuf - bb) * kk / aa;
                                        TIM_SetCompare2(TIM3, DutyfactorSet_HORIZ_LEFT);
                                }
                        }
                        else if(DutyfactorSet_HORIZ_RIGHT > (pwmBuf - bb) * kk / aa)
                        {
                                DutyfactorSet_HORIZ_RIGHT -= dutyfactor_change;
                                TIM_SetCompare2(TIM3, DutyfactorSet_HORIZ_RIGHT);
                                 if (DutyfactorSet_HORIZ_RIGHT <= (pwmBuf - bb) * kk / aa)
